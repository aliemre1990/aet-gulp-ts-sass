...
function ModuleMarkupFile(filePath, configuration) {
    ...
    this.additionalStyleReferences = [];
    this.additionalScriptReferences = [];
...
ModuleMarkupFile.prototype.create = function (prodMode) {
    ...
    this.vendorStyleFiles.forEach(x => {
        styleReferences +=
            `<link href="/${path.relative(this.configuration.publicDirectory, x.getOutputPath())}" rel="stylesheet" />` + '\r\n';
    });
    this.additionalStyleReferences.forEach(x => {
        styleReferences +=
    ...
    if (this.moduleStyleFile)
        styleReferences +=
            `<link href="/${path.relative(this.configuration.publicDirectory, this.moduleStyleFile.getOutputPath())}" rel="stylesheet" />` + '\r\n';
    ...
    this.additionalScriptReferences.forEach(x => {
        scriptReferences +=
