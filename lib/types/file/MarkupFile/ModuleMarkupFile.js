...
function setExpressionsRecursively(expressionContent) {
    var data;

    expressions = helper.findExpressions(expressionContent, this.configuration.expressions);

    for (var postFix in expressions) {
        var expressions = expressions[postFix];
        var adapter = this.configuration.expressions.find(x => x.type === postFix);
        var isRecursive = this.configuration.expressions.find(x => x.type === postFix);
        if (!data) {
            data = {};
        }
        if (isRecursive) {
...
ModuleMarkupFile.prototype.render = async function (req) {
    ...
    for (let i = 0; i < markupFileChain.length; i++) {
        ...
        if (i === markupFileChain.length) {
            ...
            data[this.configuration.scriptExpressions] = scriptReferences;
            data[this.configuration.styleExpression] = styleExpression;
        }

        /**
         * {text:[text1],content:[content1],link:[link1]}
         */
        var expressions = helper.findExpressions(markupFileChain[i].content, this.configuration.expressions);

        for (var postFix in expressions) {
            var expressions = expressions[postFix];
            var adapter = this.configuration.expressions.find(x => x.postFix === postFix);
            var isRecursive = this.configuration.expressions.find(x => x.postFix === postFix);
            if (isRecursive) {
                var expressionContents = await adapter(expressions, req);
                for (var expName in expressionContents) {
                    var expContent = expressionContents[expName];
        ...
        if (markupFileChain[i].dataProviders && markupFileChain[i].dataProviders.length > 0) {

            for (var dataProvider of markupFileChain[i].dataProviders) {
                data = { ...data, ...(await dataProvider(req)) };
            }
        }
        ...
        if (markupFileChain[i].componentMarkupFiles) {
            ...
            for (var componentMarkupFile of markupFileChain[i].componentMarkupFiles) {
                ...
                partials[`${componentMarkupFile.getExpression()}`] = partialCompiled;
            }
        }
...
ModuleMarkupFile.getStyleReferences = function () {
    ...
    while (curr) {
        if (curr.componentMarkupFiles) {
            curr.componentMakrupFiles.forEach(x => {
    ...
    componentStyleFiles.forEach(x => {
        styleReferences =
            `<link href="/${x}${this.prodMode ? `?v=${new Date().getTime()}` : ''}" rel="stylesheet" />\r\n`;
    })
...
ModuleMarkupFile.prototype.getScriptReferences = function () {
    ...
    componentScriptFiles.forEach(x => {
        scriptReferences +=
            `<script src="/${x}${this.prodMode ? `?v=${new Date().getTime()}` : ''}"></script>\r\n`;
    });
