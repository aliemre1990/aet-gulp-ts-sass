...
Controller.prototype.construct = function () {
    ...
    */
    ...
    for (let vendorScriptName in this.configuration.vendorScripts) {
        ...
        if (vendorScript.copyEntirely) {
            let relativePaths = [];
            for (var referencePaths of vendorScript.relativePathsOfReferencesThatCopiedEntirely) {
                if (this.prodMode) {
                    if (referencePaths.minRelativePath) {
                        relativePaths.push(referencePaths.minRelativePath);
                    } else {
                        relativePaths.push(referencePaths.standardRelativePath);
                    }
                } else {
                    if (referencePaths.standardRelativePath) {
                        relativePaths.push(referencePaths.standardRelativePath);
                    } else {
                        relativePaths.push(referencePaths.minRelativePath);
                    }
            ...
            let vendorScriptDirectory = new VendorScriptDirectory(vendorScript.directoryPathThatWillBeCopiedEntirely, vendorScriptName, relativePaths, this.configuration);
            this.vendorScriptFilesAndDirectories.push(vendorScriptDirectory);
        } else {
            ...
            this.vendorScriptFilesAndDirectories.push(vendorScriptFile);
    ...
    for (let vendorStyleName in this.configuration.vendorStyles) {
        ...
        if (vendorStyle.copyEntirely) {
            let relativePaths = [];
            for (var referencePaths of vendorStyle.relativePathsOfReferencesThatCopiedEntirely) {
                if (this.prodMode) {
                    if (referencePaths.minRelativePath) {
                        relativePaths.push(referencePaths.minRelativePath);
                    } else {
            ...
            let vendorStyleDirectory = new VendorStyleDirectory(vendorStyle.directoryPathThatWillBeCopiedEntirely, vendorStyleName, relativePaths, this.configuration);
            this.vendorStyleFilesAndDirectories.push(vendorStyleDirectory);
        } else {
            ...
            this.vendorStyleFilesAndDirectories.push(vendorStyleFile);
        ...
        if (layoutModuleConfiguration) {
            ...
            if (!bag.vendorScriptsExplicitlySet) {
                if (layoutModuleConfiguration.includeVendorScripts) {
                    bag.vendorScriptFilesAndDirectories = this.vendorScriptFilesAndDirectories
                    ...
                    bag.vendorScriptFilesAndDirectories = [];
                    bag.vendorScriptsExplicitlySet = true;
            ...
            if (!bag.vendorStylesExplicitlySet) {
                if (layoutModuleConfiguration.includeVendorStyles) {
                    bag.vendorStyleFilesAndDirectories = this.vendorStyleFilesAndDirectories
                    ...
                    bag.vendorStyleFilesAndDirectories = [];
                    bag.vendorStylesExplicitlySet = true;
    ...
    for (let moduleMarkupFile of this.moduleMarkupFiles) {
        ...
        let bag = {
            ...
            vendorScriptFilesAndDirectories: this.vendorScriptFilesAndDirectories.slice(),
            vendorScriptsExplicitlySet: false,
            vendorStyleFilesAndDirectories: this.vendorStyleFilesAndDirectories.slice(),
        ...
        moduleMarkupFile.vendorScriptFilesAndDirectories = bag.vendorScriptFilesAndDirectories;
        moduleMarkupFile.vendorStyleFilesAndDirectories = bag.vendorStyleFilesAndDirectories;
        ...
        moduleMarkupFile.vendorScriptFilesAndDirectories.forEach(x => x.parentModuleMarkupFiles.push(moduleMarkupFile));
        moduleMarkupFile.vendorStyleFilesAndDirectories.forEach(x => x.parentModuleMarkupFiles.push(moduleMarkupFile));
...
Controller.prototype.build = function (cb) {
    ...
    for (let vendorScriptFile of this.vendorScriptFilesAndDirectories)
        promises.push(vendorScriptFile.transfer());

    for (let vendorStyleFile of this.vendorStyleFilesAndDirectories) {
        promises.push(vendorStyleFile.transfer());
    }
