 ...
 * @constructor
 */
function Controller(configurationPath, prodMode) {
    ...
    */
    ...
    for (let moduleMarkupFile of this.moduleMarkupFiles) {
        ...
        let bag = {
            ...
            vendorScriptFiles: this.vendorScriptFiles,
            vendorScriptsExplicitlySet: false,
            vendorStyleFiles: this.vendorStyleFiles,
...
Controller.prototype.watch = function () {
    ...
    mainWatcher.on('all', (event, filePath, stats) => {
        ...
        switch (event) {
            ...
            case 'unlink':
                ...
                break;
            case 'change':
                if (filePath === 'clientBuild/configuration.js') {
    ...
    moduleStyleWatcher.on('change', (filePath, stats) => {
        ...
        this.setImportedChildren(moduleScriptFile, true);
    ...
    libraryScriptWatcher.on('change', (filePath, stats) => {
        ...
        if (scriptFiles.length > 0)
            scriptFiles.forEach(x => x.build(this.prodMode));
    });
    libraryStyleWatcher.on('change', (filePath, stats) => {
        ...
        if (styleFiles.length > 0)
            styleFiles.forEach(x => x.build(this.prodMode));
    });
...
Controller.prototype.getStyleFileOutputPath = function (styleType, styleName) {
    let filePath;
    switch (styleType) {
...
Controller.prototype.getScriptFileOutputPath = function (scriptType, scriptName) {
    let filePath;
    switch (scriptType) {
        case this.configuration.referenceParentPaths.vendorScripts:
            ...
            //     else {
            //         if (scriptName === path.basename(this.configuration.systemjsPath) ||
            //             scriptName === path.basename(this.configuration.systemjsNamedRegistersPath))
            //             filePath = path.join(this.configuration.outputDirectories.vendorScripts, scriptName);
            //     }
            // }
            break;
        // case this.configuration.referenceParentPaths.standaloneLibraryScripts:
        //     filePath = this.standaloneScriptLibraryScriptFiles
        //         .find(x => {
        //             if (x.getOutputPath)
        //                 return path.basename(x.getOutputPath()) === scriptName;
        //             else
        //                 return false;
        //         }).getOutputPath();
        // break;
...
Controller.prototype.setImportedChildren = function (file, isStyle) {
    ...
    let importedChildren = importedFilePaths
