...
/**
 * Gets the handlebars content and type-postfix mapping and gets all expressions that match the postfixes.
 * Returns an object where the keys are postFixes and values are array of expressions.
 * @param {string} content 
 * @param {Array<{type:string,postFix:string}>} typePostFixMapping 
 */
function findExpressions(content, typePostFixMapping) {
    var result = {};
    var parsed = Handlebars.parse(content);

    function traverseBlockStatementRecursively(statement) {
        for (var subStatement of statement.program.body) {
            if (subStatement.type === 'MustacheStatement') {
                if (subStatement.path.original.endsWith(typePostFixMap.postFix)) {
                    if (!result[typePostFixMap.postFix]) {
                        result[typePostFixMap.postFix] = [];
                    }

                    if (!result[typePostFixMap.postFix].includes(subStatement.path.original)) {
                        result[typePostFixMap.postFix].push(subStatement.path.original);
                    }
                }
    ...
    for (var statement of parsed.body) {
        ...
        for (var typePostFixMap of typePostFixMapping) {
            if (statement.type === 'MustacheStatement') {
                if (statement.path.original.endsWith(typePostFixMap.postFix)) {
                    if (!result[typePostFixMap.postFix]) {
                        result[typePostFixMap.postFix] = [];
                    }

                    if (!result[typePostFixMap.postFix].includes(statement.path.original)) {
                        result[typePostFixMap.postFix].push(statement.path.original);
                    }
                }
            } else if (statement.type === 'BlockStatement') {
                traverseBlockStatementRecursively(statement);
            }
        }
    }
    return result;
