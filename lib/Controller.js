...
Controller.prototype.construct = function () {
    ...
    this.componentMarkupFiles = helper
        .findFilesRecursively(path.join(this.configuration.projectDirectory, this.configuration.sourceDirectories.components), this.configuration.validMarkupExtensions)
    ...
    this.templateMarkupFiles = helper
        ...
        .map(x => {
            ...
            result.dataProviders = this.templateDataProviderMapping[result.getFileName()];
    ...
    // Get component names for future processing
    var components = {};
    ...
    for (let layoutModuleMarkupFile of this.layoutModuleMarkupFiles) {
        ...
        for (var componentName in components) {
            var componentMarkupFile = components[componentName];
            if (layoutModuleMarkupFile.content.indexOf(componentMarkupFile.getComponentPlaceHolder()) >= 0) {
                layoutModuleMarkupFile.componentMarkupFiles.push(componentMarkupFile);
            }
        }

        layoutModuleMarkupFile.dataProviders = this.layoutModuleDataProviderMapping[layoutModuleMarkupFile.getModuleName()];
    }
    ...
    for (let moduleMarkupFile of this.moduleMarkupFiles) {
        ...
        for (var componentName in components) {
            var componentMarkupFile = components[componentName];
            if (moduleMarkupFile.content.indexOf(componentMarkupFile.getComponentPlaceHolder()) >= 0) {
                moduleMarkupFile.componentMarkupFiles.push(componentMarkupFile);
            }
        }

        moduleMarkupFile.dataProviders = this.moduleDataProviderMapping[moduleMarkupFile.getModuleName()];
    }
}
/**
 * Builds files.
